...
impl Writer {
    ...
    /// Write a ScreenChar to the buffer using volatile write to prevent
    /// the compiler from optimizing it away.
    fn write_volatile(&mut self, row: usize, col: usize, sc: ScreenChar) {
        let ptr = &mut self.buffer.chars[row][col] as *mut ScreenChar;
        unsafe { core::ptr::write_volatile(ptr, sc); }
    }

    /// Read a ScreenChar from the buffer using volatile read.
